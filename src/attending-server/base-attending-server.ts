...
class AttendingServerV2 {
  ...
  timers: Collection<ServerTimerType, NodeJS.Timeout | NodeJS.Timer> =
    new Collection();
  ...
  private _queues: Collection<CategoryChannelId, HelpQueueV2> =
    new Collection();
  ...
  static async create(user: User, guild: Guild): Promise<AttendingServerV2> {
    ...
    if (
      guild.members.me.roles.highest.comparePositionTo(guild.roles.highest) < 0
    ) {
      const owner = await guild.fetchOwner();
      await owner.send(
        SimpleEmbed(
    ...
    const externalServerData = externalBackup?.find(
  ...
  async createQueue(newQueueName: string): Promise<void> {
    const existingQueues = this._queues.find(
  ...
  async dequeueWithArgs(
    ...
    await Promise.all<void | Message<boolean>>([
      ...this.serverExtensions.map(
        ...
        (extension) =>
          extension.onDequeueFirst(this, student as Readonly<Helpee>)
      ),
      student.member.send(
        SimpleEmbed(
          `It's your turn! Join the call: ${invite.toString()}`,
   ...
   */
  async closeAllClosableQueues(
    helperMember: GuildMember
  ): Promise<Required<Helper>> {
  ...
  private async createHierarchyRoles(): Promise<void> {
    const existingRoles = (await this.guild.roles.fetch())
      .filter(
        (role) => role.name !== this.user.username && role.name !== "@everyone"
      )
      .map((role) => role.name);
    ...
    const studentRole = this.guild.roles.cache.find(
  ...
  private async createClassRoles(): Promise<void> {
    const existingRoles = new Set(
      this.guild.roles.cache.map((role) => role.name)
    );
