...
class AttendingServerV2 {
  ...
  static async create(user: User, guild: Guild): Promise<AttendingServerV2> {
    ...
    if (externalServerData !== undefined) {
      console.log(
        `${FgCyan}Found external backup for ${guild.name}.` + ` Restoring.${ResetColor}`
      );
    }
      ...
      setInterval(
        async () =>
          await Promise.all(
            ...
            console.error(
              new PeriodicUpdateError(`${err.name}: ${err.message}`, 'Server')
            )
          ),
  ...
  async onMemberJoinVC(member: GuildMember, newVoiceState: VoiceState): Promise<void> {
    const memberIsStudent = this._activeHelpers.some(helper =>
      helper.helpedMembers.some(helpedMember => helpedMember.member.id === member.id)
    );
  ...
  async onMemberLeaveVC(member: GuildMember): Promise<void> {
    ...
    await Promise.all<void | boolean | Message<boolean>>([
      ...this.serverExtensions.map(extension => extension.onStudentLeaveVC(this, member)),
  ...
  async createQueue(newQueueName: string): Promise<void> {
    ...
    if (queueWithSameName !== undefined) {
      return Promise.reject(new ServerError(`Queue ${newQueueName} already exists`));
    }
  ...
  async dequeueGlobalFirst(helperMember: GuildMember): Promise<Readonly<Helpee>> {
    ...
    if (helperVoiceChannel === null) {
      return Promise.reject(new ServerError(`You need to be in a voice channel first.`));
    }
    ...
    if (nonEmptyQueues.size === 0) {
      return Promise.reject(
        new ServerError(`There's no one left to help. You should get some coffee!`)
      );
    }
    ...
    await Promise.all<void | Message<boolean>>([
      ...this.serverExtensions.map(extension => extension.onDequeueFirst(this, student)),
      student.member.send(
        SimpleEmbed(
  ...
  async dequeueWithArgs(
    ...
    if (specificQueue !== undefined) {
      ...
      const queue = this._queues.find(queue =>
        queue.students.some(student => student.member.id === targetStudentMember.id)
      );
    ...
    if (student === undefined) {
      ...
      return Promise.reject(new ServerError('Dequeue with the given arguments failed.'));
    }
    this._activeHelpers.get(helperMember.id)?.helpedMembers.push(student);
    // this api call is slow
    await helperVoiceChannel.permissionOverwrites.create(student.member, {
  ...
  async openAllOpenableQueues(helperMember: GuildMember, notify: boolean): Promise<void> {
    ...
    await Promise.all(
      this.serverExtensions.map(extension => extension.onHelperStartHelping(this, helper))
    );
  }
  ...
  async removeStudentFromQueue(
    ...
    await this._queues.get(targetQueue.parentCategoryId)?.removeStudent(studentMember);
    await Promise.all(
      this.serverExtensions.map(extension => extension.onServerRequestBackup(this))
    );
  }
  ...
  async addStudentToNotifGroup(
    ...
    await this._queues.get(targetQueue.parentCategoryId)?.addToNotifGroup(studentMember);
  }
    ...
    if (targetQueue !== undefined) {
      ...
      if (
        ...
        !helperMember.roles.cache.some(
          role => role.name === targetQueue.queueName || role.name === 'Bot Admin'
        )
      ) {
        return Promise.reject(
          new ServerError(
    ...
    if (existingHelpCategory.length === 0) {
      ...
      await Promise.all(
        commandChConfigs.map(async roleConfig => {
          ...
          await commandCh.permissionOverwrites.create(this.guild.roles.everyone, {
            SendMessages: false,
            ViewChannel: false
          });
          await Promise.all(
            this.guild.roles.cache
              ...
              .map(roleWithViewPermission =>
                commandCh.permissionOverwrites.create(roleWithViewPermission, {
                  ViewChannel: true
                })
              )
          );
  ...
  private async sendCommandHelpChannelMessages(
    ...
    await Promise.all(
      allHelpChannels.map(
        async ch =>
          await ch.messages.fetch().then(messages => messages.map(msg => msg.delete()))
      )
    );
    // send new ones
    await Promise.all(
